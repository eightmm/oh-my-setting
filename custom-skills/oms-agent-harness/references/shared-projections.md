...
| Repository scope | Normalize once, then apply the same Bash-compatible literal/glob membership and forbidden precedence; batch changed paths through one compiled rule set | change guard, proposal validation, admission, autonomy fences | Ad hoc string-prefix checks, one interpreter per changed path, or assuming two different glob spellings are subset-related. Admission and landing still retain separate authority. |
...
| Plan actionability | One snapshot for task count, non-vacuity, dependency readiness, claim expiry, and reviewed PROJECT digest/state | agent-plan, state, inbox, runtime, autonomy loops | Treating stored `ready` as executable, ignoring contract drift, treating an empty plan as completed, or using only `claimed_at` when the canonical anchor has a fallback. Existing lease cleanup is not a new claim. |
