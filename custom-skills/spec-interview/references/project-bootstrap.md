...
6. The agent-facing files (`AGENTS.md`, `CLAUDE.md`, `PROJECT.md`) are hidden
   from git locally by the template step, so a public repo carries no harness
   residue. Confirm with `oms project-private status`; apply it after a late
   `git init`, and use `--no-private` only when the project wants them
   committed. They are local-only: a fresh clone or `git clean -x` drops them,
   so re-run `oms apply-project-template` there.
