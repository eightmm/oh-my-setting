...
| 실험 | Run ledger (`run-ledger.sh`) | 학습 실행 wrapper: pre-flight `check.sh` 게이트, 중복 실행 경고, run당 한 줄씩 `docs/EXPERIMENTS.jsonl`에 기록, `--metrics`로 eval 스칼라 저장; 각 row에 게이트 결정(passed/skipped/recorded/none) 기록, 적용 가능한 게이트 skip은 `--reason` 필수(기록·secret 스캔) |
