...
- **Multi-agent 워크플로** — review(diff 게이트, ML 체크리스트, debate, 합성),
  ...
  artifact는 `.oms/artifacts/`에 저장되고 compact JSONL index로 최근/실패
  결과를 찾을 수 있다.
...
- **재사용 코드 source** — 개인 GitHub 같은 신뢰 source의 파일을 registry에
  등록하고 `gh`로 가져온다. 기본 overwrite는 막고 `.oms/code-sources.jsonl`에
  provenance를 남긴다.
- **공유 메모리 + task 핸드오프** — compact cross-agent 메모리(`.oms/memory/`)와
  active task packet(`.oms/task/current.md`)으로 세 agent 누구든 같은 작업을
  이어감; agent run 결과·loop 상태·반복 실패·검증 단계도 task packet에 남기고,
  task close 시 결론이 메모리로 승격.
- **ML 가드레일** — registered research-run wrapper와 실험 run ledger
  (pre-flight `check.sh` 게이트, 중복 실행 경고), 스캐폴딩되는 `ml_smoke.py`
  one-batch 계약, ML 특화 리뷰 게이트,
