...
| Multi-agent | Review (`multi-agent-review.sh`) | 세 로컬 모델이 diff를 병렬 리뷰; ML pre-training 게이트(`--ml`), debate 라운드, finding별 verdict |
...
| Multi-agent | 안전장치 (내장) | 외부 CLI 호출 전 outbound prompt scrub (credential, key, 머신/cluster 상세 감지 시 호출 차단); 주입 context는 fence; diff는 sanitize |
