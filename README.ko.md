...
0.4 release tag가 발행되기 전에 `main`의 0.4 코드를 설치하려면 edge channel을
명시한다:
...
0.4 installer의 기본값은 최소 구성이다. 규칙·스킬·dispatcher와 이미 설치된 provider용
hook만 연결하며, provider CLI 설치, `.bashrc` 수정, machine snapshot, update
timer, star prompt는 실행하지 않는다. 0.4 edge의 all-in-one 구성이 필요하면:
...
발행된 안정판은 GitHub Release installer를 사용한다. v0.4.0부터 installer에
release tag가 내장되어 다시 실행해도 미래 `main` commit으로 이동하지 않는다.

```bash
curl -fsSL https://github.com/eightmm/oh-my-setting/releases/latest/download/install.sh | bash
```

`--ref v0.4.0` 또는 `OH_MY_SETTING_REF=v0.4.0`으로 tag를 고정할 수 있으며,
branch와 commit도 허용한다. 자세한 계약은 [릴리스 문서](docs/RELEASE.md)와
[0.4 마이그레이션](docs/MIGRATION-0.4.md)에 있다.
...
적용하지 말고 업데이트가 있는지만 확인해줘.
마지막으로 성공한 oh-my-setting 업데이트로 롤백해줘.
