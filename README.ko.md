...
그 다음 아무 디렉토리에서나 — 빈 디렉토리든, 진행 중이던 코드든,
oh-my-setting 적용된 프로젝트든 — coding agent를 열고 말한다:

```text
이 프로젝트 시작하자.
```

agent가 상태를 감지해서 라우팅한다: 빈 디렉토리 → spec 인터뷰 →
`PROJECT.md` → 템플릿 → 안전한 skeleton → doctor; 기존 repo → 코드 먼저
읽고 템플릿 적용, `PROJECT.md`는 코드에서 채우고 빈칸만 인터뷰; 진행 중
프로젝트 → `PROJECT.md` 읽고 doctor 실행, 상태와 다음 스텝 보고. shell에
직접 칠 명령이 없다. 전체 버전은 [Agent 시작 문구](#agent-시작-문구) 참고.
...
- 프로젝트 memory source log: `.oms/memory/shared.md`
- 프로젝트 prompt context: `.oms/memory/pins.md` + `.oms/memory/summary.md`
- 전역 memory source log: `~/.oh-my-setting/local/agent-memory.md`
- provider prompt에는 full source log가 아니라 compact memory를 기본 주입한다.
- 안전장치: credential, private key, 로컬 머신 경로, cluster detail,
  프로젝트 private path처럼 보이는 note는 append/pin 단계에서 거부한다.
- 반드시 지켜야 하는 규칙은 여전히 `AGENTS.md`, repo 문서, script, hook에
  둔다. Shared memory는 soft recall이다.
- 현재 작업 handoff packet은 `.oms/task/current.md`에 둔다. provider prompt에
  기본 주입되어 Codex, Claude Code, Antigravity가 전체 대화를 다시 보지 않고도
  같은 작업을 이어갈 수 있다.
- task를 close하면 한 줄 결론(goal + next step)이 프로젝트 shared memory로
  승격된다 — 다음 세션이 결론에서 시작한다 (`OMS_AGENT_TASK_CLOSE_MEMORY=0`으로
  끔).
...
이 repo pin으로 저장해줘: 현재 작업은 compact memory로 cross-agent context 유지.
이 repo compact shared harness memory 보여줘.
```
...
agent는 내부적으로 `agent-memory.sh`, `agent-task.sh`, `agent-run.sh`를 실행한다.
`agent-run.sh`는 읽기 전용 질문은 `agent-call.sh`, 쓰기 작업은
`multi-agent-delegate.sh`로 라우팅해 격리된 git worktree에서 patch로 회수한다.
ML repo는 `agent-ml-context.sh` compact digest도 기본으로 붙는다.
...
라인은 외부 review 전에 제외된다. 최종 outbound prompt도 다시 검사해서
민감해 보이는 context가 남아 있으면 호출을 차단한다.
...
`fast`는 CPU 전용 60초 미만, `ml-smoke`는 one-batch ML interface smoke,
`gpu`는 짧은 GPU smoke로 Slurm 머신에서는 srun으로 감싼다. 위임된 worker는
ML repo로 감지되고 `ml-smoke`가 있으면 `check.sh ml-smoke`를 우선 실행하고,
없으면 `check.sh fast`를 실행한다.
...
ledger 실행마다 가드 두 개가 먼저 돈다:

- Pre-flight gate: `scripts/check.sh`가 있으면 launch 전에 실행한다
  (`ml-smoke` 구현 시 우선, 아니면 `fast`). 계약이 실패하면 launch를 중단한다
  — 자기 smoke도 못 깨는 프로젝트에 GPU 시간을 쓰지 않는다. `--no-gate` 또는
  `OMS_RUN_LEDGER_GATE=0`으로 생략.
- 중복 경고: 동일한 이전 실행(같은 commit, 같은 diff hash, 같은 명령)이 있으면
  직전 결과를 출력하고 나서 실행한다. `OMS_RUN_LEDGER_DUP=0`으로 끔.
...
새 프로젝트·기존 repo·진행 중 프로젝트 모두 문구 하나:
...
짧은 트리거 `이 프로젝트 시작하자.`도 같은 동작을 한다 — 라우팅은 prompt가
아니라 설치된 spec-interview skill에 들어 있다.
