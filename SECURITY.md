...
- The MCP server reads harness state, and `oms_peer_start` can launch an
  ...
  data-handling rules therefore still apply to MCP-triggered calls.
...
- Lifecycle events contain bounded identifiers and state, not prompts,
  ...
  once by an exact action. An explicit expiry pass can terminalize unused
  grants; stale reservations require a dry-run-first reconcile and retain an
  unknown outcome instead of being guessed successful or failed. Patch-land
  reservations defer to its intent-and-tree recovery before terminalization.
