...
| Experiments | Data manifest (`data-manifest.sh`) | Fingerprints dataset splits; `leakage` flags train/eval ID overlap and `check` flags split drift (stores only hashes, never rows) |
