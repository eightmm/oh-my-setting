...
operations remain in operation totals but are excluded from per-model results
because their combined cost, duration, and outcome cannot truthfully belong to
one model. Anonymous provider-default routes also have no row. The table shows
calls, verified/failed counts, success rate, tokens, cost, and mean duration,
so a routing choice can be argued from outcomes rather than from a table
someone maintains. `oms skill-forge eval --record` is the
