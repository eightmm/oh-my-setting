...
| `tick` | Hourly unattended sweep of registered repos (`oms tick register`): journal sync, attempt reconcile, threads idle over 7d closed, Codex plugin cache refreshed when stale; gc only with `OMS_TICK_GC=1`. `install` wires a systemd user timer or a cron line this checkout owns. |
