...
| `land` | One detached job per landing: gate, `git push --no-verify`, `oms update` when the repo is the harness checkout, CI poll, receipt beside its gate log under `$XDG_STATE_HOME/oh-my-setting/land/<repo-slug>/` read by `oms land status`. Refuses dirty or diverged trees and never pushes a HEAD that moved during the gate. |
| `tick` | Hourly unattended sweep of registered repos (`oms tick register`): journal sync, attempt reconcile, threads idle over 7d closed, active goal-less task packets closed after 7d, idle all-done plans retired after 14d, and mechanically recovered artifact timeouts resolved. `OMS_TICK_RETIRE=0` opts out of task/plan retirement; gc remains opt-in with `OMS_TICK_GC=1`. Each receipt and `swept` line reports `tasks_closed`, `plans_retired`, and `artifacts_resolved`; a stale Codex plugin cache is refreshed. `install` wires a systemd user timer or a cron line this checkout owns. |
