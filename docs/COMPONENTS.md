...
| `land` | One detached job per landing: gate, `git push --no-verify`, `oms update` when the repo is the harness checkout, CI poll, receipt under `.oms/land/` read by `oms land status`. Refuses dirty or diverged trees and never pushes a HEAD that moved during the gate. |
