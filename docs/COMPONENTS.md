...
| Multi-agent | Delegate (`multi-agent-delegate.sh`) | Runs a write task in an isolated git worktree, verifies it there, returns a reviewable patch; `--apply` only on a clean tree; `--task-id` stamps the run for plan lineage; `--repair N` feeds a verify failure and the rejected patch back to the same worker for bounded self-correction; `--plan-task` couples the run to a plan task (release on failure, review/done on success) |
...
| Experiments | Run spine (`oms-run.sh`) | Canonical `run_id` join index over the run tools — `show`/`ls`, `diff` two runs (config/env/metric deltas), `validate` JSONL/schema, `timeline` merges every `.oms` stream plus the ledger into one time-ordered "what did the agents do here" view |
