...
agent_memory_sensitive_re() {
  printf '%s\n' '((^|[^A-Za-z0-9])[A-Za-z0-9_]*(t[o]ken|s[e]cret|passw(or)?[d]|credentia[l]s?|(ap[i]|s[e]cret|privat[e])[-_ ]?(ke[y]|t[o]ken)|aws_s[e]cret_access_[k]ey)[[:space:]]*[:=]|auth[o]rization:[[:space:]]+[^[:space:]]+|bear[e]r[[:space:]]+[A-Za-z0-9._-]{10,}|[a-z][a-z0-9+.-]*://[^[:space:]/:@]+:[^*[:space:]@/][^[:space:]@/]*@[^[:space:]/]+|(^|[^A-Za-z0-9_-])ey[J][A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}($|[^A-Za-z0-9_-])|gh[pousr]_[A-Za-z0-9_]{20,}|githu[b]_pa[t]_[A-Za-z0-9_]{20,}|npm_[A-Za-z0-9]{36,}|h[f]_[A-Za-z0-9]{34,}|glpa[t]-[A-Za-z0-9_-]{20,}|[sr]k_(liv[e]|tes[t])_[A-Za-z0-9]{16,}|AI[z]a[0-9A-Za-z_-]{35}|hook[s]\.slac[k]\.com/service[s]/|disc[o]rd(app)?\.com/api/webhook[s]/|machin[e][[:space:]]+[^[:space:]]+[[:space:]]+logi[n][[:space:]]+[^[:space:]]+[[:space:]]+passwor[d][[:space:]]+[^[:space:]]+|(^|[^A-Za-z0-9_])s[k]-[A-Za-z0-9_-]{10,}|xox[bap]-[A-Za-z0-9-]{10,}|AK[I]A[0-9A-Z]{16}|-----BE[G]IN|/hom[e]/[^[:space:]]+|/User[s]/[^[:space:]]+|\.ss[h]/|\.aw[s]/|clust[e]r[[:space:]]*[:=]|partiti[o]n[[:space:]]*[:=]|nodelis[t][[:space:]]*[:=]|sbatc[h][[:space:]]+--partition)'
}
