...
# journal, reconciles dead attempts, closes threads idle past a threshold, and
# refreshes a stale Codex plugin cache; gc stays opt-in (OMS_TICK_GC=1) because
# its retention floor is a per-repo decision. `install` wires an hourly
# user-level timer (systemd, cron fallback) so sessions start on a swept tree.
...
run        Sweep the given repos, or every registered one: journal sync,
           attempt reconcile, stale-thread close, optional gc, and a Codex
...
case "$IDLE_DAYS$STEP_TIMEOUT" in *[!0-9]*) echo "error: OMS_TICK_* values must be integers" >&2; exit 2 ;; esac
...
sweep_repo() {  # sweep_repo ROOT -> one summary line; receipt in .oms/tick/last.json
  local root="$1" oms="$ROOT/scripts/oms" journal=0 reconcile=0 gc=skipped closed=() id
  ...
  python3 - "$root/.oms/tick/last.json" "$journal" "$reconcile" "$gc" "${closed[@]}" <<'PY'
import datetime, json, os, sys, tempfile
path, journal, reconcile, gc, *closed = sys.argv[1:]
...
os.replace(tmp, path)
PY
  echo "swept $root: journal=$journal reconcile=$reconcile gc=$gc threads_closed=${#closed[@]}"
}
...
status_tick() {
  ...
  while IFS= read -r root; do
    ...
    if [ -f "$root/.oms/tick/last.json" ]; then
      printf '%s  last: %s\n' "$root" "$(python3 -c 'import json,sys; r=json.load(open(sys.argv[1])); print(r["ran_at"], "journal_rc=%s threads_closed=%d" % (r["journal_rc"], len(r["threads_closed"])))' "$root/.oms/tick/last.json" 2>/dev/null || echo '?')"
