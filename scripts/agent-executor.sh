...
if [ "$ACTION" = "create" ]; then
  ...
  if [ -n "$PLAN_TASK" ]; then
    ...
    values="$(printf '%s' "$plan_json" | python3 -c 'import json,sys;d=json.load(sys.stdin);print("\t".join([d.get("id",""),d.get("lease_id",""),",".join(d.get("allowed_paths",[])),",".join(d.get("forbidden_paths",[])),d.get("verify",""),d.get("role","")]))')"
